...
import {Button, ConfigProvider, Form, InputNumber, message, Select} from "antd";
...
const ModuleManagementAirflow = () => {
    ...
    const error = () => {
        messageApi.open({
            type: 'error',
            content: `Ошибка подключения к серверу ${wsAPI}/air-flow/`,
        });
    };
    ...
    useEffect(() => {
        ...
        if (!client) {
            client = new W3CWebSocket(`${wsAPI}/air-flow/`);
                ...
                <div className={styles.innerContainer}>
                    <ConfigProvider theme={{
                        ...
                        <Form className={styles.form} onFinish={(values) => {
                            ...
                            <Form.Item label="Cредняя расчитанная потребность в кислороде: "
                                       className={styles.formItem}>
